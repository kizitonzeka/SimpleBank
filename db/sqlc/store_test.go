...
//Test if deadlock occurs when reverse concurrent transfer operation is executed on the accounts
func TestTransferTxDeadlock(t *testing.T) {
	...
	// If this loop block is removed, the balance prints unexpected results. Why?
	for i := 0; i < n; i++ {
		err := <-errs
		require.NoError(t, err)
	}
